...
def main(reset_goal):
	...
	if reset_goal:
		goal = unlocks.get_next_goal(Items.Carrot)
		if goal:
			goal_name = goal[0]
			goal_costs = goal[1]
			...
			goal_costs = {Items.Carrot: 100000}
		total_costs = costs.get_required_costs_by_goal(goal_costs)

		if len(total_costs) == 0:
			# print("No depending costs.")
			req = {}
			req["entities"] = {Entities.Carrot: goal_costs[Items.Carrot]}
			req["items"] = {}
			req["need"] = goal_costs[Items.Carrot]
			set_need(req)
